...
def load_actors(credits, id):
    ...
    # in tmdb, sex is 0,1,2. so I used this list in a way that 0,1,2 are the indexes of the correct values 
...
def load_tmdb_movies():
    ...
    movie_count = len(mycursor.fetchall())
    mycursor= CONNECTOR.cursor()
    i = 1
    while movie_count < MAX_SIZE:
        print("Working on movie", i)
        try:
            p = movie.details(i)
        except:
            print("skipping id ", i)
        ...
        try:
            rating= mov['user rating']
        except:
            rating="NULL"

        mycursor.execute(INSERT_INTO_MOVIES, [parseImdbId(
            p.imdb_id), p.title, rating, p.runtime, p.release_date, p.revenue])
        CONNECTOR.commit()
        try:
...
def load_movie_genres(movie_details):
    ...
    for g in genres:
        mycursor.execute(INSERT_INTO_MOVIE_GENRES, [
                         parseImdbId(movie_details.imdb_id), g.id])
        CONNECTOR.commit()
    mycursor.close()
...
def load_movie_actors(movie_id, cast):
    ...
    for actor in cast:
        mycursor.execute(INSERT_INTO_MOVIE_ACTORS, [parseImdbId(movie_id), actor['id']])
        CONNECTOR.commit()
    mycursor.close()
